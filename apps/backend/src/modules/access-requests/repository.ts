...
export async function create(userId: string, campaignId: string): Promise<AccessRequestRow | null> {
  ...
  const { rows } = await pool.query<AccessRequestRow>(
    `INSERT INTO access_requests (user_id, campaign_id)
     VALUES ($1, $2)
     ON CONFLICT (user_id, campaign_id) WHERE status = 'pending'
     DO NOTHING
     RETURNING id, user_id, campaign_id, status, requested_at, resolved_at, resolved_by, note`,
    [userId, campaignId],
  );
  return rows[0] ?? null;
}
...
export async function resolve(
  ...
  try {
    ...
    // 2) If approved, grant campaign access
    if (status === "approved") {
      await client.query(
        ...
        [row.user_id, row.campaign_id, permTierra, permDigital],
      );
    }
