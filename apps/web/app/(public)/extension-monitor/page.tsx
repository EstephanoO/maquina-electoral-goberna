...
/**
 * GOBERNA — Extension Monitor (Condensed War Room)
 *
 * Single-page dashboard combining:
 * - 6 phone cards (WA send activity per Vasquez phone)
 ...
 *
 * Auto-refreshes every 30s (phones/CMS) + SSE for classifications.
 * All data from production API via public endpoints (x-campaign-id auth).
 */
...
import { ClassificationMetrics, ClassificationFeed } from "./clasificaciones/_components";
...
// ══════════════════════════════════════════════════════════════════════
// PHONE COMPONENTS (compact)
// ══════════════════════════════════════════════════════════════════════

function OpRow({ op, max }: { op: ExtensionMonitorOperator; max: number }) {
  const pct = max > 0 ? (op.wa_sent / max) * 100 : 0;
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
      <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, background: "rgba(255,200,0,0.12)", border: "1.5px solid rgba(255,200,0,0.30)", color: G.gold, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 900 }}>
        {(op.full_name[0] ?? "?").toUpperCase()}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: G.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{name}</div>
        <div style={{ height: 2, borderRadius: 1, background: "rgba(255,255,255,0.06)", overflow: "hidden", marginTop: 2 }}>
          <div style={{ height: "100%", width: `${pct}%`, borderRadius: 1, background: "linear-gradient(90deg,#FFC800,#FFE066)", transition: "width 0.6s ease" }} />
        </div>
      </div>
      <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 900, color: G.gold }}>{op.wa_sent}</span>
    </div>
  );
}
...
function PhoneScreen({ phone, slotName }: { phone: ExtensionMonitorPhone; slotName: string }) {
  ...
  const ops = [...phone.operators].filter(o => o.wa_sent > 0).sort((a, b) => b.wa_sent - a.wa_sent);
  const maxOp = ops[0]?.wa_sent ?? 0;
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: active ? "linear-gradient(175deg,#0f1d2f 0%,#091422 60%,#050c15 100%)" : "linear-gradient(175deg,#0a1420 0%,#060d18 60%,#030810 100%)", display: "flex", flexDirection: "column", fontFamily: FONT, overflow: "hidden" }}>
      <div style={{ height: "7%", flexShrink: 0, display: "flex", alignItems: "flex-end", justifyContent: "flex-end", padding: "0 10% 5px" }}>
        {active && <div style={{ width: 6, height: 6, borderRadius: "50%", background: G.gold, boxShadow: `0 0 6px ${G.gold}`, animation: "gobPulse 2.5s ease-in-out infinite" }} />}
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "4% 8% 3%", gap: "3%", overflow: "hidden" }}>
        <div style={{ fontSize: "clamp(11px,4%,15px)", fontWeight: 900, color: active ? G.gold : G.textDim, letterSpacing: "-0.2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{slotName}</div>
        {active ? (
          <>
            <div style={{ display: "flex", gap: "4%" }}>
              {[{ v: phone.wa_sent, l: "Env" }, { v: phone.unique_contacts, l: "Cont" }].map(({ v, l }) => (
                <div key={l} style={{ flex: 1, padding: "8% 0", background: "rgba(255,200,0,0.07)", borderRadius: "12%", border: "1px solid rgba(255,200,0,0.18)", display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <span style={{ fontSize: "clamp(16px,7%,26px)", fontWeight: 900, lineHeight: 1, color: G.gold }}>{v}</span>
                  <span style={{ fontSize: "clamp(7px,2.5%,10px)", fontWeight: 700, color: G.goldDim, marginTop: "8%" }}>{l}</span>
                </div>
              ))}
            </div>
            <div style={{ flex: 1, overflow: "hidden" }}>
              {ops.slice(0, 4).map(op => <OpRow key={op.operator_id} op={op} max={maxOp} />)}
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: "2%", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
              <span style={{ fontSize: "clamp(7px,2.5%,10px)", color: G.textDim }}>Ult</span>
              <span style={{ fontSize: "clamp(7px,2.5%,10px)", fontWeight: 700, color: "rgba(255,200,0,0.55)" }}>{fmtRel(phone.last_event_at)}</span>
            </div>
          </>
        ) : (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: "clamp(8px,3%,11px)", color: G.textDim }}>En espera</span>
          </div>
        )}
      </div>
...
function PhoneCard({ phone, slotName }: { phone: ExtensionMonitorPhone; slotName: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
      <div style={{ width: "100%", filter: active ? "drop-shadow(0 0 16px rgba(255,200,0,0.22)) drop-shadow(0 8px 30px rgba(0,0,0,0.7))" : "drop-shadow(0 4px 18px rgba(0,0,0,0.6))", transition: "filter 0.5s ease", borderRadius: 36 }}>
        <Iphone frameColor="#000000" screenColor={active ? "#0f1d2f" : "#0a1420"}>
          <PhoneScreen phone={phone} slotName={slotName} />
        </Iphone>
      </div>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.6px", textTransform: "uppercase", color: active ? G.gold : G.textDim }}>{slotName}</div>
        <div style={{ fontSize: 9, fontWeight: 700, color: active ? G.green : G.textDim, marginTop: 2 }}>{active ? "ACTIVO" : "ESPERA"}</div>
      </div>
    </div>
  );
}
...
// ══════════════════════════════════════════════════════════════════════
// CMS FUNNEL BAR (compact horizontal bar)
// ══════════════════════════════════════════════════════════════════════
  ...
  const loadPhones = useCallback(async () => {
    ...
    try {
      const [monitorRes, statsRes] = await Promise.all([
        getExtensionMonitor(CAMPAIGN_ID),
        fetch(`/api/cms/stats?campaign_id=${CAMPAIGN_ID}`, { credentials: "same-origin" }).then(r => r.json()).catch(() => null),
      ]);
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        ::-webkit-scrollbar-thumb { background: rgba(255,200,0,0.15); border-radius: 2px; }
      `}</style>

      <div style={{ minHeight: "100vh", background: `radial-gradient(ellipse 90% 40% at 50% -5%, rgba(22,57,96,0.28) 0%, transparent 65%), ${G.bg}`, color: G.text, fontFamily: FONT }}>

        {/* ── Header ── */}
        <div style={{ background: "rgba(9,22,38,0.94)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,200,0,0.14)", padding: "0 24px", position: "sticky", top: 0, zIndex: 50 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 52, gap: 16 }}>
            {/* Left: logo */}
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 28, height: 28, borderRadius: 6, background: G.gold, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 900, color: G.navyDark }}>G</div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 900, color: G.gold, letterSpacing: "2.5px" }}>GOBERNA</div>
                <div style={{ fontSize: 9, color: "rgba(255,200,0,0.38)", fontWeight: 600 }}>Extension Monitor</div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: G.gold, lineHeight: 1 }}>{totalSent}</div>
                <div style={{ fontSize: 9, fontWeight: 700, color: G.goldDim, textTransform: "uppercase", letterSpacing: "0.5px" }}>Enviados</div>
              </div>
              <div style={{ width: 1, height: 24, background: "rgba(255,255,255,0.07)" }} />
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: G.text, lineHeight: 1 }}>{activeCount}<span style={{ fontSize: 12, color: G.textMid }}>/6</span></div>
                <div style={{ fontSize: 9, fontWeight: 700, color: G.textMid, textTransform: "uppercase", letterSpacing: "0.5px" }}>Activos</div>
              </div>
              <div style={{ width: 1, height: 24, background: "rgba(255,255,255,0.07)" }} />
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: G.gold, lineHeight: 1 }}>{classTotal}</div>
                <div style={{ fontSize: 9, fontWeight: 700, color: G.goldDim, textTransform: "uppercase", letterSpacing: "0.5px" }}>Clasificaciones</div>
              </div>
              <div style={{ width: 1, height: 24, background: "rgba(255,255,255,0.07)" }} />
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <div style={{
                  ...
                  boxShadow: `0 0 5px ${sseStatus === "connected" ? G.green : sseStatus === "connecting" ? G.gold : G.red}`,
                  animation: sseStatus === "connected" ? "gobPulse 2.5s ease-in-out infinite" : "none",
                }} />
                <span style={{ fontSize: 10, fontWeight: 700, color: sseStatus === "connected" ? G.green : sseStatus === "connecting" ? G.gold : G.red }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {lastAt && <span style={{ fontSize: 10, color: G.textDim, fontVariantNumeric: "tabular-nums" }}>{lastAt.toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</span>}
              <button
                type="button"
                ...
                style={{
                  ...
                  fontSize: 11, cursor: (phoneLoading || classLoading) ? "default" : "pointer", fontWeight: 800,
                }}
              >
                {(phoneLoading || classLoading) ? "\u00B7\u00B7\u00B7" : "\u21BB"}
              </button>
            </div>
          </div>
        </div>
        ...
        {(phoneError || classError) && (
          <div style={{ margin: "12px 24px 0", background: "rgba(239,83,80,0.07)", border: "1px solid rgba(239,83,80,0.22)", borderRadius: 8, padding: "8px 14px", fontSize: 12, color: G.red }}>
            {phoneError || classError}
          </div>
        )}
        ...
        <div style={{ padding: "20px 24px 16px", maxWidth: 1400, margin: "0 auto" }}>
          ...
          {/* Row 2: CMS funnel bar + Classification dashboard */}
        ...
        {/* ── Footer ── */}
        <div style={{ borderTop: `1px solid ${G.border}`, padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 10, color: G.textDim, fontWeight: 600 }}>
            GOBERNA \u00B7 Extension Monitor \u00B7 Phones {POLL_PHONES_MS / 1000}s \u00B7 SSE {sseStatus === "connected" ? "activo" : "inactivo"} \u00B7 Poll {sseStatus === "connected" ? POLL_SSE_MS / 1000 : POLL_FALLBACK_MS / 1000}s
          </span>
          <span style={{ fontSize: 10, color: G.textDim }}>{lastAt ? `${lastAt.toLocaleTimeString("es-PE")}` : "\u2014"}</span>
        </div>
      </div>
    </>
  );
