...
function OperatorCard({ op, maxWorked }: { op: CmsMetricsOperator; maxWorked: number }) {
  ...
  const barPct = maxWorked > 0 ? (totalWorked / maxWorked) * 100 : 0;
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: totalWorked > 0 ? "var(--goberna-blue-600, #2563eb)" : "var(--color-text-tertiary)", lineHeight: 1 }}>
            {totalWorked}
          </div>
...
function TimeCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", paddingTop: 10, borderTop: "1px solid var(--color-border)" }}>
...
export default function CmsMetricsPage() {
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    setLoading((prev) => !metrics ? true : prev); // Only show spinner on first load
    setError(null);
    try {
  ...
  // Sort operators: most active first
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1100, margin: "0 auto", fontFamily: FONT }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--color-text-tertiary)", margin: "4px 0 0" }}>
            {activeCampaign?.name ?? "Campana"} &middot; {g.total} contactos
          </p>
        </div>
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 28, alignItems: "stretch" }}>
        ...
        <FunnelStep
          ...
          total={g.total}
          color="#9ca3af"
          icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="21 8 21 21 3 21 3 8" /><rect x="1" y="3" width="22" height="5" /></svg>}
          isLast
        />
      </div>
      ...
      >
        <div style={{ flex: "1 1 200px", background: "#f0fdf4", borderRadius: 12, padding: "16px 20px", border: "1px solid #bbf7d0" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#166534", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>
            Tasa de Contacto
          </div>
        ...
        <div style={{ flex: "1 1 200px", background: "var(--goberna-blue-50, #eff6ff)", borderRadius: 12, padding: "16px 20px", border: "1px solid var(--goberna-blue-200, #bfdbfe)" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--goberna-blue-900)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>
            Eficiencia Global
          </div>
          <div style={{ fontSize: 32, fontWeight: 800, color: "var(--goberna-blue-600, #2563eb)", lineHeight: 1 }}>
      ...
      <h2 style={{ fontSize: 15, fontWeight: 800, color: "var(--color-text-primary)", margin: "0 0 14px", letterSpacing: "0.02em" }}>
        AGENTES DIGITALES
        <span style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-tertiary)", marginLeft: 8 }}>
          {sortedOperators.length} activo{sortedOperators.length !== 1 ? "s" : ""}
        </span>
      </h2>

      {sortedOperators.length === 0 ? (
        <div style={{ background: "var(--color-surface)", borderRadius: 12, padding: 32, textAlign: "center", border: "1px solid var(--color-border)", color: "var(--color-text-tertiary)", fontSize: 14 }}>
