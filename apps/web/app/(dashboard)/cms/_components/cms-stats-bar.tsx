...
const STAT_ITEMS: { key: keyof CmsStats; label: string; color: string; bg: string }[] = [
  { key: "total", label: "Total", color: "text-text-secondary", bg: "bg-surface-active" },
  ...
  { key: "archivados", label: "Archivados", color: "text-text-tertiary", bg: "bg-surface-hover" },
];

export function CmsStatsBar({ stats, loading }: CmsStatsBarProps) {
  return (
    <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/80 bg-surface/80 backdrop-blur-sm shrink-0 overflow-x-auto">
      {STAT_ITEMS.map((item) => (
        <div
          key={item.key}
