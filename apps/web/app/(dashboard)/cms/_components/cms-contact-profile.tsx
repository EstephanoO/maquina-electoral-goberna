...
export function CmsContactProfile({ contact, onSave, onClose, saving }: CmsContactProfileProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-surface border-l border-border/80 w-[380px] shrink-0">
      {/* Header */}
      <div className="shrink-0 px-4 py-3 border-b border-surface-active flex items-center justify-between">
        <div className="min-w-0">
          <h3 className="text-sm font-bold text-text-primary truncate">{contact.nombre || "Sin nombre"}</h3>
          <div className="flex items-center gap-2 mt-0.5">
            <span className={`text-[11px] font-medium ${statusInfo.color}`}>{statusInfo.label}</span>
            {contact.telefono && (
              <span className="text-[11px] text-text-tertiary">{contact.telefono}</span>
            )}
          </div>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="p-1.5 rounded-lg text-text-tertiary hover:bg-surface-active hover:text-text-secondary transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
        {/* Context card */}
        <div className="rounded-xl bg-surface-hover p-3 space-y-2">
        ...
        <div className="rounded-xl bg-surface-hover p-3 space-y-3">
          <SectionTitle>Clasificacion de contacto</SectionTitle>

          {/* Progress bar */}
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-text-tertiary">Puntaje: {score}/{MAX_SCORE}</span>
            ...
            {SIGNAL_ITEMS.map((item) => (
              <label
                ...
                className={`flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer transition-colors ${
                  flags[item.key] ? "bg-surface shadow-sm" : "hover:bg-surface/60"
              ...
              >
                <input
                  ...
                  disabled={isArchived}
                  className="w-3.5 h-3.5 rounded border-border-strong text-[var(--goberna-blue-600)] focus:ring-[var(--goberna-blue-500)]"
                />
        ...
        {/* Editable notes */}
        <div className="rounded-xl bg-surface-hover p-3 space-y-2.5">
          ...
          />
          <div>
            <label htmlFor="cms-profile-comentarios" className="block text-[10px] font-semibold text-text-tertiary uppercase tracking-wide mb-1">
              Comentarios
            </label>
            <textarea
              ...
              rows={3}
              className="w-full text-[12px] px-2.5 py-1.5 rounded-lg border border-border bg-surface outline-none focus:border-[var(--goberna-blue-400)] resize-none placeholder:text-text-tertiary disabled:bg-surface-active disabled:text-text-tertiary"
            />
          </div>
        </div>
      </div>
      ...
      {/* Footer */}
      {!isArchived && (
        <div className="shrink-0 px-4 py-2.5 border-t border-surface-active flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-3 py-1.5 rounded-lg text-[12px] font-medium text-text-tertiary hover:bg-surface-active transition-colors"
          >
            Cancelar
          </button>
          <button
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h4 className="text-[10px] font-bold text-text-tertiary uppercase tracking-wide">{children}</h4>;
}
...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex items-start justify-between gap-2">
      ...
      <span className="text-[11px] text-text-secondary font-medium text-right truncate">{value || "\u2014"}</span>
    </div>
  );
}
...
function FieldInput({ label, value, onChange, disabled, placeholder }: {
  ...
  return (
    <div>
      <label htmlFor={id} className="block text-[10px] font-semibold text-text-tertiary uppercase tracking-wide mb-1">
        {label}
      </label>
      <input
        id={id}
        ...
        className="w-full text-[12px] px-2.5 py-1.5 rounded-lg border border-border bg-surface outline-none focus:border-[var(--goberna-blue-400)] placeholder:text-text-tertiary disabled:bg-surface-active disabled:text-text-tertiary"
      />
    </div>
  );
}
