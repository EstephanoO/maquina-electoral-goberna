...
import { getMonitorTheme, MonitorThemeProvider, type MonitorThemeMode } from "./_components/theme";
...
function PhoneCard({ phone, slotName, maxSent, G }: {
  phone: ExtensionMonitorPhone; slotName: string; maxSent: number; G: ReturnType<typeof getMonitorTheme>;
}) {
...
export default function MonitorWaPage() {
  ...
  return (
    <MonitorThemeProvider mode={themeMode}>
      <style>{`
        @keyframes gobPulse {
          ...
          <div style={shouldUsePhoneCarousel ? {
            ...
            {slots.map(({ slotName, phone }) => (
              <div key={slotName} style={shouldUsePhoneCarousel ? { flex: "0 0 220px", scrollSnapAlign: "start" } : undefined}>
                <PhoneCard phone={phone} slotName={slotName} maxSent={maxPhoneSent} G={G} />
              </div>
            ))}
          </div>
