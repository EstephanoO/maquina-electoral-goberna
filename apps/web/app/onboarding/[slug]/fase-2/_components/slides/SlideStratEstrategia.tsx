...
export function SlideStratEstrategia({ data }: Props) {
  ...
  return (
    <div className="min-h-full bg-[#020a1e] flex flex-col pt-16 pb-4 px-4 sm:px-8 gap-5">
      ...
      >
        <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-green-400/60 mb-1">
          ESTRATEGIA
        </p>
        <h2 className="text-2xl sm:text-3xl font-black text-white">CÓMO GANAR SURCO · 3 PASOS</h2>
      </motion.div>
        ...
        {estrategia.pasos.map((paso, i) => {
          ...
          return (
            <motion.div
              ...
              className="rounded-xl border border-white/10 bg-white/5 p-5 flex flex-col gap-3 relative overflow-hidden"
              style={{ borderLeft: `4px solid ${paso.color}` }}
            >
              <span
                className="text-7xl font-black absolute -top-3 -right-2 opacity-10 select-none leading-none"
              ...
              <div>
                ...
                <h3 className="text-base font-black text-white leading-snug">{paso.titulo}</h3>
              </div>
              <p className="text-xs text-white/60 leading-relaxed flex-1">{paso.descripcion}</p>
              <span
                className="inline-block text-[8px] font-bold uppercase tracking-widest rounded-full px-2 py-0.5 self-start mt-auto"
                style={{ backgroundColor: paso.color + "22", color: paso.color }}
              >
                {paso.segmento}
              </span>
            </motion.div>
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-2">
              <div className="bg-white/10 rounded-lg px-3 py-1.5 text-center">
                ...
                <p className="text-[9px] text-white/40 font-normal">{box.label}</p>
              </div>
