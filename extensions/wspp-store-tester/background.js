...
// ═══════════════════════════════════════════════════════════════════════
// WSPP_SENT — incrementa contador local Y reporta al backend
// ═══════════════════════════════════════════════════════════════════════

// M-9: Added sendResponse parameter for delivery acknowledgment
// S-1 FIX: Use apiFetch() instead of raw fetch() to get automatic 401→refresh→retry.
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type !== 'WSPP_SENT') return;

  const { phone, own_number, contact_name } = msg.payload;
  ...
  chrome.storage.local.get(['wspp_count'], (data) => {
    ...
    // 2. Reportar al backend si hay sesión activa (via apiFetch con auto-refresh)
    if (phone || contact_name) {
      ...
      console.log('[WSPP] → sent event:', JSON.stringify(body));
      apiFetch('/api/cms/extension-event', {
        method: 'POST',
        body:   JSON.stringify(body),
      }).then(j => {
    ...
    // M-9: Acknowledge delivery to content.js
