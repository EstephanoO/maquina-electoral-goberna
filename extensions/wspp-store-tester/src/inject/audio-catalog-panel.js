...
function handleCatalogItemClick(audioId, label) {
  ...
  if (_previewData?.id === audioId && _previewAudio) {
    if (_previewPlaying) { _previewAudio.pause(); _previewPlaying = false; }
    else { _previewAudio.play().then(() => { _previewPlaying = true; if (_catalogPanelOpen) renderCatalogPanel(); }).catch(() => { _previewPlaying = false; }); return; }
    if (_catalogPanelOpen) renderCatalogPanel();
    return;
  }
  _destroyPreview();
...
export async function sendAudioAsPTT(audioBase64, mimeType) {
  ...
  try {
    ...
    } catch (err) { E('Step 1 chat', err); return false; }
    if (!chat) { E('Step 1 chat', 'chat is null after resolve'); return false; }
    ...
    // ── Step 4: prepRawMedia → waitForPrep ───────────────────────────
    ...
    // ── Step 11: Build + send PTT message ─────────────────────────────
    ...
    const message   = {
      ...mediaJSON, ...ephemeralFields,
      id: newMsgKey, ack: 0, from: meUser, to: chat.id,
      local: true, self: 'out', t: Math.floor(Date.now() / 1000),
      isNewMsg: true, type: 'ptt', mimetype: mime,
    };
